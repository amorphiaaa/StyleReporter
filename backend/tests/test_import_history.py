...
from app.api.routes.imports import _to_import_history_item, _to_import_run_response
...
def test_import_run_response_exposes_metadata_and_row_errors() -> None:
    import_id = UUID("f71fa28b-e681-4fb9-9252-53078bd9ecf5")
    started_at = datetime(2026, 8, 20, 18, 0, tzinfo=UTC)
    import_run = ImportRun(
        id=import_id,
    ...
    detail = _to_import_run_response(import_run)

    assert detail.import_id == import_id
    assert detail.spreadsheet_id == "synthetic-spreadsheet"
    assert detail.row_errors[0].code == "invalid_email"
    assert detail.completed_at == started_at
