...
- GOOGLE_DRIVE_OAUTH_CLIENT_JSON (OAuth Desktop client JSON or mounted path)
- GOOGLE_DRIVE_OAUTH_REFRESH_TOKEN (generated once by the local OAuth helper)
...
workspace to a canonical Drive folder. For a personal Drive, use the OAuth
refresh token generated by `tools/google_drive_oauth.py`; the service account
path is reserved for Shared Drives. The publisher creates the client folder and
